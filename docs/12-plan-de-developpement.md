...
Le reste de la tranche, dans l'ordre où il se livrera : la **normalisation** des libellés et la recherche de candidats — les pluriels s'y traitent en interrogeant d'abord le libellé brut, l'index de l'ANSES gardant les siens ; les six fournisseurs et leur fabrique, le prompt en asset et l'intercepteur de redaction ; les clés en `EncryptedSharedPreferences` avec le bouton **Tester** et le compteur de coût ; les deux écrans de saisie et le repli IA groupé ; enfin la contribution à Open Food Facts ([D70](11-decisions.md#d70--contribuer-à-open-food-facts-entre-en-tranche-6-parce-que-la-couverture-nest-pas-la-même-partout---validée)).
